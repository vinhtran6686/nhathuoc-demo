...
// function isCartLocation(params) {
//     return false;
// }

//check radio giới tính anh/chị - hình thức nhận hàng
var getInfoFormUser = function (element, loadDefault = false) {
    ...
    if (isCartLocation() && element != '.cart-bottom__form') {
        radioChecked = $(`${element} .tab-item.active`);
        isRadioChecked = radioChecked.hasClass('active');
        if (isRadioChecked) {
            nameRadio = radioChecked.find('.label-text').text().trim();
...
function eventChangeInputTax() {
    ...
    var labelTax = $('.js-check-tax label');

    labelTax.removeClass('f-w-500');
...
function formValidateUser(loadDefault = false) {
    var inputNameUser = $('.form-input[name="ho_ten"]'),
        nameUser = inputNameUser.val();

    var isRadioChecked = getInfoFormUser('.cart-bottom__form', loadDefault);
...
function userFocusInput() {
    ...
    var formGroup = $('.cart-bottom__form .form > .form-groups .js-info');
    var formReceiving = $('.form-receiving');
    ...
    formGroup.each(function () {
        ...
        $that.click(function (e) {
            if (formReceiving.is(':hidden') && $('.table-receive').is(':hidden')) {
                formReceiving.slideDown();
            }
        });
        ...
        _that.click(function (e) {
            ...
            var checkAttrButtonCity = () => {
                ...
                if (isAttrRegionCity) {
                    ...
                    (widtWindow < 768) ?
                        labelTexxtDropdown.text('Tỉnh/Thành')
                        :
...
function getAddressUser() {
    ...
    function showTableReceive() {
        ...
        $('.table-receive, .cart__info-user').fadeIn();
    ...
    function showInfoReceive(address, date, note, time) {
        ...
        if(isMobile()){
            setTimeout(() => {
                $("html, body").animate({scrollTop: $('.cart-bottom__payments').offset().top }, 500, 'swing');
            }, 1000)
        }
    ...
    if (listTab[1] === itemTabs) {
        //button dropdown
        var buttonCity = $('.tabs.active .select__region_city .dropdown-button');
        buttonDistrict = $('.tabs.active .select__region_district .dropdown-button');
        buttonWard = $('.tabs.active .select__region_ward .dropdown-button');
...
function isInvalidCity(tokenCity) {
    ...
    if (webTabletMobile) {
        $('.select__region_city').children('.alert').find('span').text('Tỉnh/Thành');
        $('.select__region_district').children('.alert').find('span').text('Quận/Huyện');
    }
...
function dropdownSearch() {
    ...
    inputSearchDropdown.each(function () {
        ...
        $(document).on('click', '.select--bankcode, .select--bankcodevnpay', function () {
...
function vuLoadMoreShop() {
    ...
    $('.cs-map').click(function(){
        $('.address-item .radio').removeClass('checked');
    });
}
...
//             var heightCTAMB = parseInt(ctaMobile.height());
//             browserWindow.scroll(function () {
//                 var windowScrollTop = parseInt(browserWindow.scrollTop() + offsetTopSticky);
//                 var ctaMBTop = parseInt($('.cart-cta-mb').offset().top + heightCTAMB);
//                 if (windowScrollTop >= ctaMBTop) {
//                     $('.sticky-top').addClass('fixed-bottom');
//                 } else {
